...
void drawStar(TFT_eSPI &TFTscreen, int16_t x, int16_t y, uint8_t radius, uint16_t color) {
    // Draw a simple star
    TFTscreen.drawCircle(x, y, radius, color);  // Center circle
    for (int angle = 0; angle < 360; angle += 36) {  // Draw lines at every 36 degrees
        int x_end = x + radius * cos(radians(angle));
        int y_end = y + radius * sin(radians(angle));
        TFTscreen.drawLine(x, y, x_end, y_end, color);
    }
}
  void drawFPS(TFT_eSPI &TFTscreen, long time)
  {
